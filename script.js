...
async function takeSnapshot() {
  ...
  // Load and draw the brand logo in the top-left corner
  const logoImg = await loadImage('logo.png');
  if (logoImg) {
    const logoWidth = snapshotCanvas.width * 0.1; // 10% of canvas width
    const logoHeight = (logoImg.height / logoImg.width) * logoWidth;
    ctx.drawImage(logoImg, 10, 10, logoWidth, logoHeight); // Draw logo at (10, 10)
  }
